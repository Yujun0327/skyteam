...
export function connectRoom(code: string): Transport {
  const topic = `skyteam/v1/${code.toUpperCase()}`
  ...
  const clients: MqttClient[] = BROKERS.map((url, i) => {
    const client = mqtt.connect(url, {
      ...
      clientId: `skyteam_${tag}_${i}_${randHex(3)}`,
      clean: true,
      keepalive: 30,
    ...
    client.on('connect', () => {
      ...
      console.log(`[skyteam] broker up: ${url}`)
      for (const fn of joinHandlers) fn(url)
    })
    client.on('close', () => {
      for (const fn of leaveHandlers) fn(url)
    })
    client.on('error', (err) => console.warn(`[skyteam] broker error ${url}:`, err.message))
  ...
  return {
    send: (msg) => {
      ...
      if (payload.length > 64_000) console.warn(`[skyteam] oversized beacon: ${payload.length}B`)
